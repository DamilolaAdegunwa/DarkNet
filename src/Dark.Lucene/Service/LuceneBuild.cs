...
using Dark.Lucene.Utils;
using System.Text;
using Dark.Lucene.Attributes.Lucene;
...
namespace Dark.Lucene.Service
{
    ...
    /// 
    /// </summary>
    public class LuceneBulid<T> : ILuceneBulid<T> where T : Dark.Core.Domain.Entity.IEntity<string>
    {
        #region 1.0 设置索引目录
        ...
        public void UpdateList(List<T> ciList)
        {
            ...
            {
                //logger.Errkkor("InsertIndex异常", ex);
                throw ex;
            }
            finally
            {
        ...
        public List<T> Query(string strCondition)
        {
            ...
            try
            {
                ...
                QueryParser parser = new QueryParser(LuceneUtil.Version.LUCENE_30, property.Name, analyzer);
                //给关键字进行分词
        ...
        /// <returns></returns>
        public List<T> QueryByPage(string strCondition, PageDto page, string strFilter)
        {
            IndexSearcher searcher = null;
            try
            {
                ...
                int startIndex = (page.Page - 1) * page.PageSize;
                int endIndex = page.Page * page.PageSize;
                ...
                //获取中标的数据
                page.PageSize = docs.TotalHits;

                //PrintScores(docs, startIndex, endIndex, searcher);
                for (int i = startIndex; i < endIndex && i < page.PageSize; i++)
                {
