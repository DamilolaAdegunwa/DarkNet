...
namespace Dark.Core.Domain.Repository
{
    public class DbHelper : IDbHelper
    {
        ...
        /// <returns></returns>
        public int Insert<T>(T entity) where T : EntityBase
        {
            //1：构建插入SQL
            Type type = typeof(T);
        ...
        /// <returns></returns>
        private string GetInsertSql<T>(T model, string tableName = "") where T : EntityBase
        {
        ...
        public void InsertList<T>(List<T> insertList, string tableName = "") where T : EntityBase
        {
        ...
        /// <typeparam name="T"></typeparam>
        /// <param name="Id"></param>
        public void Delete<T>(int? Id) where T : EntityBase
        {
            Type type = typeof(T);
        ...
        /// <returns></returns>
        public IEnumerable<T> GetByPage<T>(PageDto page) where T : EntityBase
        {
            Type type = typeof(T);
          
            //1.构建SQL
            string strSQL = $"SELECT * FROM dbo.[{type.Name}]  ORDER BY {page.Sort} offset {page.SkipCount} rows fetch next {page.PageSize} rows only;";
            //2：执行SQL
            return DbCommand<List<T>>(strSQL, (cmd) =>
           {
               //1：创建读取对象
        ...
        public void Update<T>(T entity, Dictionary<string, object> changeData) where T : EntityBase
        {
            Type type = typeof(T);
